...
$ZONE_MGR --zone Julian --temperature 19

# Example 3: Set zone by ID
$ZONE_MGR -z 1 -t 21

# Example 4: Turn off multiple zones
$ZONE_MGR --zone Julian --zone Dharma --disable
...
$ATHOME julian > /dev/null || $ZONE_MGR --zone Julian --limit 8 --quiet
$ATHOME dharma > /dev/null || $ZONE_MGR --zone Dharma --limit 8 --quiet

# Option 2: Simple cron job (run every hour)
# Limit Julian's room to 8°C during work hours (9-17) on weekdays
# Add to crontab:
# 0 9-17 * * 1-5 /usr/bin/python3 /path/to/zone-manager.py --zone Julian --limit 8 --quiet

# Option 3: Night mode - limit all zones to 16°C at night
# Add to crontab:
# 0 23 * * * /usr/bin/python3 /path/to/zone-manager.py --zone Julian --zone Dharma --limit 16 --quiet

# ============================================================
# Systemd Timer Example (alternative to cron)
# ============================================================

# File: /etc/systemd/system/tado-limit-julian.service
# [Unit]
# Description=Limit Tado temperature for Julian's room
# 
# [Service]
# Type=oneshot
# Environment="TADO_API_KEYS=your-api-key-here"
# ExecStart=/usr/bin/python3 /path/to/zone-manager.py --zone Julian --limit 8 --quiet
# User=youruser

# File: /etc/systemd/system/tado-limit-julian.timer
# [Unit]
...
# Enable: sudo systemctl enable --now tado-limit-julian.timer
